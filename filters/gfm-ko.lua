local wiki_prefix = "https://ko.wikipedia.org/wiki/"
...
local wiki_path = "../wikipedia.ko"
if not file_exists( wiki_path .. "/README.md") then
  wiki_path = "/mnt/t/chinapedia/wikipedia.ko"
end
...
function Link(el)
  ...
  elseif istarts_with(el.target, ":Category:") then
    local c = string.sub(el.target, 1 + #":Category:")
    if not category_exists(c) then
        el.target = "https://ko.wikipedia.org/wiki/Category:" .. c
        return el
    end
    el.target = "../Category/" .. c
  elseif istarts_with(el.target, "MediaWiki:") then
    ...
    if not special_page_exists("MediaWiki", c) then
        el.target = "https://ko.wikipedia.org/wiki/MediaWiki:" .. c
        return el
    end
...
function Image(el)
  return pandoc.Link(el.caption, "https://ko.wikipedia.org/wiki/File:" .. el.src, el.title)
end
